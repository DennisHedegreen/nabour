...
class NabourV2Tests(unittest.TestCase):
    def test_pair_registry_keeps_one_active_pair_and_two_staged_pairs(self):
        ...
        self.assertFalse(specs["se_no"].active)
    ...
    def test_se_no_stays_honestly_blocked(self):
        pair_spec = get_pair_spec("se_no")
        summaries = summarize_pair_coverage(pair_spec)
        counts = [summary.municipality_count for summary in summaries.values()]
